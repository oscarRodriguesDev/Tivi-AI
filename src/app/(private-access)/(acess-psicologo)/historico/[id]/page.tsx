...
<>
      ...
      <div className="p-6">
        ...
        <ul className="space-y-2">
          {historicos.map((h) => (
            <li key={h.id} className="relative border p-4 rounded-lg shadow-sm bg-gray-50">
              {/* Botão de deletar */}
              <span
                onClick={() => {handleDelete(h.id) }}
                className="absolute top-2 right-2 text-red-500 hover:text-red-700 font-bold text-base cursor-pointer transition duration-200 ease-in-out hover:scale-110 active:scale-95 select-none"
